...
export const generateDesignWithFallback = async (
  ...
  const providers: { id: AIModel; name: string; fn: () => Promise<string> }[] = [
    { id: 'gemini', name: 'Gemini Standard (Free)', fn: () => generateGemini(prompt, style, bgColor, refImageBase64, mode, aspectRatio, 'gemini-2.5-flash-preview-image') },
    { id: 'nano-banana', name: 'Banana Flash (Fast)', fn: () => generateGemini(prompt, style, bgColor, refImageBase64, mode, aspectRatio, 'gemini-2.5-flash-preview-image') },
    { id: 'nano-banana-pro', name: 'Banana Pro (HD)', fn: () => generateGemini(prompt, style, bgColor, refImageBase64, mode, aspectRatio, 'gemini-3-pro-image') },
