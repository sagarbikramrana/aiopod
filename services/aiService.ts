...
export const generateDesignWithFallback = async (
  ...
  const providers: { id: AIModel; name: string; fn: () => Promise<string> }[] = [
    { id: 'nano-banana', name: 'Nano Banana (Flash)', fn: () => generateGemini(prompt, style, bgColor, refImageBase64, mode, aspectRatio, 'gemini-2.5-flash-image') },
    { id: 'nano-banana-pro', name: 'Nano Banana Pro', fn: () => generateGemini(prompt, style, bgColor, refImageBase64, mode, aspectRatio, 'gemini-3-pro-image-preview') },
    { id: 'gemini', name: 'Gemini Standard', fn: () => generateGemini(prompt, style, bgColor, refImageBase64, mode, aspectRatio, 'gemini-2.5-flash-image') },
